...
t = data[:,0]
x = data[:,1]
y = data[:,2]+4#The data will be shifted 4 units to avoid negative values
...
#plt.grid()
#plt.show()
C = C_range[Chi2_y.index(min(Chi2_y))]-4#The parameter C is centered back
print("Parameter C:", C,
      "Evaluated in ["+str(C_range[0])+"; "+str(C_range[-1])+"]")

y = y-4#The y data are shifted -4 to turn back how they were originally
...
b = {}
C = {}""".format(A,a,B,b,C)
plt.plot(t,fy(t,A,a,B,b,C),label=LabelGraph)
